...
So: do the OAuth flow ourselves for the full scope set, keep the result in the
keychain, and hand Claude Code a short-lived mirror before each run. Everything
Claude Code needs, nothing it has to write.

Storage
-------
The master copy lives in the iOS keychain, through the ccauth-keychain helper
installed beside this file: a generic-password item, service
com.andi.claude-code-native.ccauth, kSecAttrAccessibleAfterFirstUnlock
ThisDeviceOnly. The item is wrapped in the device keybag, so it does not
survive "Erase All Content and Settings", it is not included in backups or
device-to-device restores, and -- living on the data volume -- it survives iOS
updates and jailbreak reinstalls that would wipe the jailbreak root.

What Claude Code itself reads is still a file: ~/.claude/.credentials.json,
recreated before every launch. The mirror deliberately carries the ~8 h access
token only; the long-lived refresh token never touches the file system.

CCAUTH_PLAINTEXT=1 skips the keychain entirely and keeps the master in
~/.claude/ccauth.json (0600) -- the pre-keychain behaviour, kept as an escape
hatch. Any plain-text master found while the keychain is enabled is imported
into it and deleted.
...
paranoid: an exclusive lock so two concurrent starts cannot both refresh, a
keychain write that updates the item in place rather than delete-then-add, and
no destructive step anywhere on failure -- a refresh that fails leaves the
existing credential exactly as it was.
"""
...
OAUTH_TOKEN = os.path.join(HOME, ".claude", "oauth-token")

# The keychain helper, installed beside this file and signed with the same
# entitlements.plist as the patched binary. The keychain-access-groups entry in
# that plist is what authorizes SecItem access: without it SecItemAdd returns
# -34018 (verified on device -- an ad-hoc binary gets exactly that). The helper
...
def _to_credentials(resp, previous=None):
    """Shape a token response into the credential Claude Code reads.

    Unknown fields from an existing credential are preserved: the format is
    Anthropic's, not ours, and dropping a field we do not recognise would be a
    silent downgrade.
    """
...
def _write_atomic(path, creds):
    ...
    with os.fdopen(fd, "w") as f:
        json.dump(creds, f)
        f.flush()
        os.fsync(f.fileno())          # what we write must survive a crash
    os.replace(tmp, path)
    os.chmod(path, 0o600)
...
# --- the keychain master -----------------------------------------------------
#
# The helper's protocol: `get` prints the blob (exit 3 when nothing is stored,
# 1 on error), `set` reads the blob on stdin (update in place when the item
# already exists), `del` is idempotent.

def _kc(*args, data=None):
    try:
        return subprocess.run([HELPER, *args], input=data, capture_output=True, timeout=30)
    except (OSError, subprocess.SubprocessError) as e:
        raise AuthError(
            "keychain helper unusable (%s). Reinstall the package; or set "
            "CCAUTH_PLAINTEXT=1 to keep the credential in a 0600 file instead." % e)


def _kc_error(p):
    msg = (p.stderr or p.stdout or b"").decode("utf-8", "replace").strip()
    return "keychain error: %s" % (msg or "exit status %d" % p.returncode)


def _read_master():
    """The keychain master, or None. AuthError if the helper is broken."""
    if not USE_KEYCHAIN:
        return _read_file(STORE)
    p = _kc("get")
    if p.returncode == 3:
        return None
    if p.returncode != 0:
        raise AuthError(_kc_error(p))
    try:
        return json.loads(p.stdout)
    except ValueError:
        raise AuthError("stored credential is not valid JSON -- run claude-login again")


def _write_master(creds):
    if not USE_KEYCHAIN:
        _write_atomic(STORE, creds)
        return
    p = _kc("set", data=json.dumps(creds).encode())
    if p.returncode != 0:
        raise AuthError(_kc_error(p))
    # A plain-text master left over from before this version still holds the
    # current refresh token; importing it means removing it.
...
def read():
    """The master, importing older file-based copies on the way.

    Order: the keychain first; then ~/.claude/ccauth.json from a pre-keychain
    install (imported into the keychain and removed right here); then
    .credentials.json, which is how a credential copied from a desktop arrives.
    Either way the copy holding the refresh token ends up out of the file
    system on the next write.
    """
    if USE_KEYCHAIN:
        creds = _read_master()
        if creds:
            return creds
        legacy = _read_file(STORE)
        if legacy:
            _write_master(legacy)
            return legacy
        return _read_file(CRED)
...
def materialise(creds=None):
    ...
    The mirror carries the access token only. Claude Code's own refresh-in-place
    already fails on iOS -- that is the whole reason this module exists -- so
    handing it the long-lived refresh token buys nothing and leaves the one
    secret that matters lying in a file. A session that outlives its token is
    fixed by starting claude again.
    """
    creds = creds or read()
    if not creds:
        return False
    published = {k: v for k, v in creds.items() if k != "ccauth"}
    oauth = dict(published.get("claudeAiOauth") or {})
...
def launch_hook():
    """Everything the wrapper needs before exec'ing Claude Code.

    Returns (signed_in, error). Error is non-fatal by design: a failed refresh
    still leaves the current access token, which Claude Code can use; a broken
    keychain leaves nothing to publish, and signed_in comes back False so the
    wrapper points at claude-login.
    """
    try:
        _, err = refresh_if_needed()
        if err:
            return True, err
        creds = read()
        if creds:
...
def logout():
    """Remove every stored copy of the credential. Returns what was removed."""
    removed = []
    if USE_KEYCHAIN:
        try:
            _kc("del")
...
def _lock():
    os.makedirs(os.path.dirname(LOCK), 0o700, exist_ok=True)
...
def exchange(code, verifier, state):
    """Trade the pasted code for tokens and store them.
