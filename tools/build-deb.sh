...
payload_digest() {
    ...
    # libshim.dylib is excluded for the same reason one step further down: a
    # compiled, ldid-signed binary is never byte-identical twice, so including
    # it would fire this guard on every single run. Its source, shim.c, ships
    # in the package and IS compared, so a real change to the shim is still
    # caught. Same for ccauth-keychain / ccauth-keychain.c.
    ( cd "$dir/x" && find . -type f \
        ! -path './control' ! -path './md5sums' \
        ! -path './var/jb/usr/local/lib/claude-native/libshim.dylib' \
        ! -path './var/jb/usr/local/lib/claude-native/ccauth-keychain' -print0 | sort -z \
